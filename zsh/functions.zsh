...
cpcat() { xclip -selection clipboard < "$1"; }
...
tm() {
    [[ -n "$TMUX" || -z "$DISPLAY" ]] && return
    echo "---- TMUX Startup ----\n[w]orkshell\n[m]odel\n[h]omeshell"
    vared -p '' -c shell
    tmux source-file ~/.tmux.conf
    case ${shell:l} in
        w) $HOME/dotfiles/tmux/tmux.startup.shell ;;
        m) $HOME/dotfiles/tmux/tmux.startup.model ;;
        h) $HOME/dotfiles/tmux/tmux.startup.home ;;
        *) tmux new-session -d -s "$shell" && tmux attach -t "$shell" ;;
    esac
}
